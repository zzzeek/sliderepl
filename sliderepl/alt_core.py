...
class Deck(object):
        ...
        def run(self, run=True, echo=True):
            ...
            for i, (display, co) in enumerate(self.codeblocks):
                ...
                if echo and not no_echo:
                    ...
                    #self.deck.console.push("\n".join(display))
                    #print("\n".join(display))

                    display = [d for d in display if d.strip()]
                    for j, l in enumerate(display):


                        if j == 0:
                            codeline = (getattr(sys, 'ps1', '>>> ') + l)
                        elif l.startswith(' '):
                            codeline = (getattr(sys, 'ps2', '... ') + l)
                        elif not l.isspace():
                            codeline = (getattr(sys, 'ps1', '>>> ') + l)
                        else:
                            codeline = l

                        if not run and i == len(self.codeblocks) - 1 and j == len(display) - 1:
                            self.deck._one_time_prompt = (codeline.rstrip(), l)
                            return
                        else:
                            self.deck.console.write(codeline)
                            self.deck.console.push(l)
                    ...
                    #print(shown)
                    #if not run and i == len(self.codeblocks) - 1:
                    #    lines = shown.split("\n")
                    #    sys.stdout.write("\n".join(lines[0:-1]))
                    #    #self.deck._one_time_prompt = lines[-1][4:] + "  # ..."
                    #else:
                    #    print(shown)
    ...
    def run(cls, path=None, **options):
        ...
        deck.console = console = code.InteractiveConsole()
        global environ
...
%% or entering the "n" or "next" command."""
    ...
    def readfunc(self, prompt=''):
        if self._one_time_prompt:
            codeline, pushline = self._one_time_prompt
            prompt = codeline + "  # ..."
        line = raw_input_(prompt)
        if self._one_time_prompt or prompt == getattr(sys, 'ps1', '>>> '):
            if self._one_time_prompt:
                self.console.push(pushline)
            self._one_time_prompt = None
            tokens = line.split()
